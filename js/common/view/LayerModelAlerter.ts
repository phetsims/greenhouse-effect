...
/**
 * LayerModelAlerter is responsible for generating alerts for changes in the Layer Model screen.  A polling method is
 * used to time the alerts and determine how model variables have changed to create an accurate description.
 *
...
export type LayerModelAlerterOptions = SelfOptions & AlerterOptions;
...
type PreviousImmediateNotificationModelState = {
  // TODO: Fill this in if we need it or blow it away.  See https://github.com/phetsims/greenhouse-effect/issues/374.
};

class LayerModelAlerter extends Alerter {

  // reference to the model, used in the methods
  private readonly model: LayerModelModel;
  ...
  // The number of times that the temperature change alert has been announced. This can be reset in order to cause an
  // alert to occur sooner, which may be desirable when the concentration or number of layers change.  Every time this
  // counter is an interval of NUMBER_OF_TERSE_TEMPERATURE_ALERTS, a more verbose temperature description is used.
  // Otherwise, a very terse alert is used. This is an attempt to reduce how much is spoken every ALERT_INTERVAL_WHILE_PLAYING
  ...
  public constructor( model: LayerModelModel, providedOptions: LayerModelAlerterOptions ) {

    const options = optionize<LayerModelAlerterOptions, SelfOptions, AlerterOptions>()( {
    ...
    model.groundLayer.atEquilibriumProperty.lazyLink( atEquilibrium => {
      if ( atEquilibrium ) {
        this.alert( TemperatureDescriber.getSurfaceTemperatureStableString(
          ...
          // TODO: Can we get away with using a fixed control mode like this?  Or do we need a new method on TemperatureDescriber?  See https://github.com/phetsims/greenhouse-effect/issues/374.
    ...
    // Whenever the number of layers changes, use the most verbose form of the temperature change alert.
    model.numberOfActiveAtmosphereLayersProperty.link( () => {
      this.useVerboseSurfaceTemperatureAlert = true;
      this.temperatureChangeAlertCount = 0;
  ...
  private saveImmediateNotificationModelState(): PreviousImmediateNotificationModelState {
    ...
    // TODO: (this is part of an earlier TODO) - Do we have immediate model state that needs alerting and thus saving?  See https://github.com/phetsims/greenhouse-effect/issues/374.
   ...
   * describe updates for things that can change while the model is paused.
   */
  public step(): void {

    // Start by alerting "immediate" model changes. These are updates that need to be described as soon as they change.

    // TODO: (part of previous TODOs) - Do we have any immediate things?  See https://github.com/phetsims/greenhouse-effect/issues/374.
    ...
    if ( this.timeSinceLastAlert > alertInterval ) {
      ...
      if ( !this.model.groundLayer.atEquilibriumProperty.value ) {
        ...
        // Describe changes to the temperature - this should get described at interval even if there is no change in
        // the number of layers.  Depending on how many times it has been spoken a terse form of it may be used.
...
greenhouseEffect.register( 'LayerModelAlerter', LayerModelAlerter );
export default LayerModelAlerter;
